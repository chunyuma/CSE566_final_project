...
def parseargs():
    parser = argparse.ArgumentParser(description="Generate a gold standard CAMI/OPAL format profile.")
    ...
    parser.add_argument('--taxonomy_path', required=True, help='Input megan class count file name.')
...
# Example for taxonomy.tsv
# accession	tax_id	name	abundance
# KT447174.1	137745	Amphinema byssoides	0.434970984428748
...
def generate_taxid_to_abundance(taxonomy_file):
    taxid_to_abundance = dict()
    with(open(taxonomy_file, 'r')) as infile:
        for line in infile:
            splits = line.strip().split('\t')
            if splits[3] == 'abundance':
                continue
            taxid = splits[1]
            if taxid not in taxid_to_abundance:
                taxid_to_abundance[taxid] = ['', "{:.5f}".format(float(splits[3])*100)]
            else:
                taxid_to_abundance[taxid][-1] = "{:.5f}".format(float(taxid_to_abundance[taxid][1]) + float(splits[3])*100)
...
if __name__ == '__main__':
    ...
    taxid_to_abundance = generate_taxid_to_abundance(args.taxonomy_path)
